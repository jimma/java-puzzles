...
public class ProfilerFallacyExample {
    ...
    private static void smallPark() {
        ...
        // - safepoint bias (it can take sampels only at safepoint: http://blog.ragozin.info/2012/10/safepoints-in-hotspot-jvm.html)
        while (!Thread.currentThread().isInterrupted()) {
    ...
    private static void doSomethingUncounted() {
        ...
        //NOTE: it tries to not cause safepoint biasing thanks to the Blackhole::consumeCPU implementation
        //      that put a safepoint on each iteration of its main loop
        while (!Thread.currentThread().isInterrupted()) {
